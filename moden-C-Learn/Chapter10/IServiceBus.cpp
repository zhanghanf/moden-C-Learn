...
TEST_CASE("AutoBrake", "[confige]") {
    REQUIRE(1 + 1 == 2);

}
TEST_CASE("initial speed is 0") {
    MockServiceBus bus;//汽车服务总线
    AutoBrake auto_brake{ bus };   
    SECTION("my testA") {
    ...
    SECTION("my TestB") {
        REQUIRE_THROWS(auto_brake.set_collision_threshold(-1));
        REQUIRE(auto_brake.get_collision_threshold() == Catch::Approx(5L));
     }
    SECTION("My test3") {
        bus.speed_update_callback(SpeedUpdata{ 100L });
        REQUIRE(auto_brake.get_speed_mps() == Catch::Approx(100L));
    }
    SECTION("My Test4") {
        bus.speed_update_callback(SpeedUpdata(100L));
        bus.car_detected_callback(CarDetected{ 100L,0L });
        REQUIRE(bus.commendpublished_count == 0L);
        REQUIRE(bus.last_brake_command.time_to_collision_s == 1L);
    }
}
